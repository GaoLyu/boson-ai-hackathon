...
class AudioExtractor:
    ...
    def extract_audio(self, video_path, audio_path=None, method=None):
        ...
        # 根据选择的方法调用相应的函数
        if method == "moviepy" and self.available_methods["moviepy"]:
            return self._extract_with_moviepy(video_path, audio_path)
        elif method == "ffmpeg_python" and self.available_methods["ffmpeg_python"]:
            return self._extract_with_ffmpeg_python(video_path, audio_path)
        elif method == "ffmpeg" and self.available_methods["ffmpeg"]:
...
if __name__ == "__main__":
    ...
    # 显示可用方法
    print("📋 可用提取方法:", extractor.get_available_methods())
    
    # 提取音频
    video_file = "sleep.mp4"  # 替换为你的视频文件
    output_audio = extractor.extract_audio(video_file)
    
    if output_audio:
        print(f"🎉 音频已保存至: {output_audio}")
